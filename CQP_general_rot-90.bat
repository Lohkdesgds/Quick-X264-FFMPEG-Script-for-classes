...
echo Please CHOOSE 1 for FULL RES (default), 2 for HALF, 3 for QUARTER, and press ENTER once to continue with that configuration.
set /p SCALE="Option: "
...
"%~dp0ffmpeg/bin/ffmpeg.exe" -y -i %IPATH% -preset slow -vsync 0 -crf 35 -ab 128k -g 120 -keyint_min 40 -bf 4 -qcomp 0.8 -rc-lookahead 80 -filter:v "transpose=3" %OPATH%
goto end

:g_half
echo Starting FFMPEG HALF RES...
"%~dp0ffmpeg/bin/ffmpeg.exe" -y -i %IPATH% -preset slow -vsync 0 -crf 32 -ab 128k -g 120 -keyint_min 40 -bf 4 -qcomp 0.8 -rc-lookahead 80 -filter:v "transpose=3, scale=in_w*0.5:in_h*0.5" %OPATH%
goto end

:g_quar
echo Starting FFMPEG QUARTER RES...
"%~dp0ffmpeg/bin/ffmpeg.exe" -y -i %IPATH% -preset slow -vsync 0 -crf 29 -ab 128k -g 120 -keyint_min 40 -bf 4 -qcomp 0.8 -rc-lookahead 80 -filter:v "transpose=3, scale=in_w*0.25:in_h*0.25" %OPATH%
goto end
