...
echo Automatic call: assuming no crop @ 68 kbps video ^& 32 kbps audio mono
echo SOURCE: %IPATH%
echo OUTPUT: %OPATH%
...
"%~dp0ffmpeg/bin/ffmpeg.exe" -y -i %IPATH% -ac 1 -preset slow -vb 68k -ab 32k -g 720 -keyint_min 360 -bf 20 -qcomp 0.9 -rc-lookahead 120 -filter:v "fps=0.5" %OPATH%
